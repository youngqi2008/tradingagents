...
class ChatDashScopeOpenAI(ChatOpenAI):
    ...
    def __init__(self, **kwargs):
        ...
        try:
            ...
            def get_env_api_key_for_provider(_provider):
                return os.getenv("DASHSCOPE_API_KEY") or os.getenv("QWEN_API_KEY")

        # Docker 部署：Compose 注入的环境变量优先于数据库 Key
        if os.getenv("DOCKER_CONTAINER", "").lower() in ("true", "1", "yes"):
            env_api_key = get_env_api_key_for_provider("dashscope")
            if env_api_key:
                api_key_from_kwargs = env_api_key
                logger.info("✅ [DashScope初始化] Docker 环境：使用 Compose 注入的 API Key")
            else:
                api_key_from_kwargs = kwargs.get("api_key")
        else:
            api_key_from_kwargs = kwargs.get("api_key")

        if api_key_from_kwargs and not is_valid_api_key(api_key_from_kwargs):
            logger.warning(
                "⚠️ [DashScope初始化] kwargs 中的 API Key 无效（可能是占位符），将回退到环境变量"
            )
        ...
        if not api_key_from_kwargs:
            env_api_key = get_env_api_key_for_provider("dashscope")
            logger.info(
                f"🔍 [DashScope初始化] 从环境变量读取 DashScope Key: {'有值' if env_api_key else '空'}"
            )
            if env_api_key:
                logger.info(
                    f"✅ [DashScope初始化] 环境变量 API Key 有效，长度: {len(env_api_key)}, 前10位: {env_api_key[:10]}..."
                )
                api_key_from_kwargs = env_api_key
            else:
                logger.warning("⚠️ [DashScope初始化] 未找到有效的 DASHSCOPE_API_KEY / QWEN_API_KEY")
        elif not os.getenv("DOCKER_CONTAINER"):
            logger.info("✅ [DashScope初始化] 使用 kwargs 中传入的有效 API Key（来自数据库配置）")
