...
def _get_async_executor() -> concurrent.futures.ThreadPoolExecutor:
    """获取用于在独立线程中运行异步代码的线程池（避免在已运行的事件循环中调用 run_until_complete）。"""
    global _async_executor
    if _async_executor is None:
        _async_executor = concurrent.futures.ThreadPoolExecutor(
            max_workers=8, thread_name_prefix="dataflow_async"
        )
    return _async_executor


def _run_async_safe(coro_factory):
    """
    安全地运行由 coro_factory() 返回的协程。
    若当前线程已有运行中的事件循环（如 FastAPI 请求内），则在独立线程中运行，避免
    ...
    if running:
        ...
        return _get_async_executor().submit(_in_thread).result()
    else:
        try:
...
def _normalize_stock_data_result(result) -> str:
    """将 get_stock_data / fallback 返回值统一为字符串（兼容误返回 tuple 的情况）"""
...
class DataSourceManager:
    ...
    def _get_data_source_priority_order(self, symbol: Optional[str] = None) -> List[ChinaDataSource]:
        ...
        # 🔥 回退到默认顺序（兼容性）
        # Docker 内 AKShare 访问东方财富常失败，优先 BaoStock
        import os
    ...
    def get_stock_data(self, symbol: str, start_date: str = None, end_date: str = None, period: str = "daily") -> str:
        ...
        try:
            ...
            result = _normalize_stock_data_result(result)
            ...
            if is_success:
                ...
                # MongoDB 路径已在 _get_mongodb_data 内尝试过备用数据源，避免重复调用
                if self.current_source == ChinaDataSource.MONGODB:
                    logger.error(f"❌ [数据来源: 所有数据源失败] 无法获取有效数据: {symbol}")
                    return result

                # 数据质量异常时也尝试降级到其他数据源
                fallback_result, fallback_source = self._try_fallback_sources(symbol, start_date, end_date)
                fallback_result = _normalize_stock_data_result(fallback_result)
            ...
            return _normalize_stock_data_result(fallback_result)
    ...
    def _get_tushare_data(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> str:
        ...
        try:
            ...
            if cached_data is not None and not cached_data.empty:
                ...
                if provider:
                    stock_info = _run_async_safe(lambda: provider.get_stock_basic_info(symbol))
            ...
            # 使用异步方法获取历史数据（在独立线程运行以避免与已有事件循环冲突）
            data = _run_async_safe(lambda: provider.get_historical_data(symbol, start_date, end_date))
    ...
    def _try_fallback_sources(self, symbol: str, start_date: str, end_date: str, period: str = "daily") -> tuple[str, str | None]:
        ...
        # Docker：确保 BaoStock 参与降级且优先于 AKShare
        import os
        if os.getenv("DOCKER_CONTAINER", "").lower() in ("true", "1", "yes"):
            docker_preferred = [
                ChinaDataSource.BAOSTOCK,
            ...
            merged = []
            for src in docker_preferred:
                if (
                    src != self.current_source
                    and src in self.available_sources
                    and src not in merged
                ):
                    merged.append(src)
            for src in fallback_order:
                if src not in merged and src != self.current_source:
                    merged.append(src)
            fallback_order = merged
    ...
    def _get_tushare_stock_info(self, symbol: str) -> Dict:
        """使用 Tushare 获取股票基本信息（供降级路径直接调用，避免循环）"""
        try:
            provider = self._get_tushare_adapter()
            if not provider:
