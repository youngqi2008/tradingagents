...
class AKShareProvider(BaseStockDataProvider):
    ...
    def _get_stock_news_direct(self, symbol: str, limit: int = 10) -> Optional[pd.DataFrame]:
        ...
        try:
            ...
            for article in articles:
                news_data.append({
                    "新闻标题": self._clean_news_text(article.get("title", "")),
                    "新闻内容": self._clean_news_text(article.get("content", "")),
            ...
            if symbol:
                ...
                if news_df is not None and not news_df.empty:
                    ...
                    for _, row in news_df.head(limit).iterrows():
                        title = self._clean_news_text(row.get('新闻标题', '') or row.get('标题', ''))
                        content = self._clean_news_text(row.get('新闻内容', '') or row.get('内容', ''))
                        summary = self._clean_news_text(row.get('新闻摘要', '') or row.get('摘要', ''))
                ...
                try:
                    ...
                    if news_df is not None and not news_df.empty:
                        ...
                        for _, row in news_df.iterrows():
                            title = self._clean_news_text(row.get('title', '') or row.get('标题', ''))
                            content = self._clean_news_text(row.get('content', '') or row.get('内容', ''))
                            summary = self._clean_news_text(row.get('brief', '') or row.get('摘要', ''))
    ...
    def _clean_news_text(self, text) -> str:
        """去掉东财等接口标题中的 <em> HTML 高亮标签。"""
        import re
