...
def create_llm_by_provider(provider: str, model: str, backend_url: str, temperature: float, max_tokens: int, timeout: int, api_key: str = None):
    ...
    elif provider.lower() in ("dashscope", "qwen", "alibaba"):
        try:
            from app.utils.api_key_utils import is_valid_api_key, get_env_api_key_for_provider
            dashscope_api_key = api_key if is_valid_api_key(api_key) else None
            if not dashscope_api_key:
                dashscope_api_key = get_env_api_key_for_provider("dashscope")
        except ImportError:
            dashscope_api_key = api_key or os.getenv('DASHSCOPE_API_KEY')

        if not dashscope_api_key:
            raise ValueError("使用阿里百炼需要设置 DASHSCOPE_API_KEY 环境变量或在数据库中配置 API Key")
...
class TradingAgentsGraph:
    ...
    def __init__(
        ...
        # 统一 provider 别名（qwen → dashscope），避免走「自定义厂家」分支
        try:
            from app.utils.api_key_utils import normalize_llm_provider
        except ImportError:
            def normalize_llm_provider(name):
                aliases = {"qwen": "dashscope", "alibaba": "dashscope", "tongyi": "dashscope"}
                return aliases.get((name or "").lower(), (name or "dashscope").lower())

        for key in ("llm_provider", "quick_provider", "deep_provider"):
            if self.config.get(key):
                self.config[key] = normalize_llm_provider(self.config[key])
        ...
        if quick_provider and deep_provider and quick_provider != deep_provider:
        ...
        elif (self.config["llm_provider"].lower() in ("dashscope", "alibaba", "qwen") or
              "dashscope" in self.config["llm_provider"].lower() or
              "阿里百炼" in self.config["llm_provider"] or
              "通义" in self.config["llm_provider"]):
            ...
            dashscope_api_key = (
                ...
                or get_env_api_key_for_provider("dashscope")
            )
            ...
            try:
                ...
                get_env_api_key_for_provider = None
                is_valid_api_key = None
                normalize_llm_provider = None

            custom_api_key = (
                self.config.get("quick_api_key") or self.config.get("deep_api_key")
            )
            if custom_api_key and is_valid_api_key and not is_valid_api_key(custom_api_key):
                custom_api_key = None

            if not custom_api_key and get_env_api_key_for_provider:
                custom_api_key = get_env_api_key_for_provider(
                    normalize_llm_provider(provider_name) if normalize_llm_provider else provider_name
                )
                if custom_api_key:
                    logger.info("✅ 从环境变量别名映射获取到 API Key")

            if not custom_api_key:
                for env_var in api_key_candidates:
                    candidate = os.getenv(env_var)
                    if candidate and (not is_valid_api_key or is_valid_api_key(candidate)):
                        custom_api_key = candidate
                        logger.info(f"✅ 从环境变量 {env_var} 获取到 API Key")
                        break
