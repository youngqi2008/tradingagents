...
class StockDataPreparer:
    ...
    @staticmethod
    def _unwrap_stock_data_result(result) -> str:
        """统一历史数据返回值（兼容 fallback 误返回 tuple 的情况）"""
        if isinstance(result, tuple):
            return str(result[0]) if result and result[0] is not None else ""
        return str(result) if result else ""

    def _parse_stock_name_from_info(self, stock_info: str, stock_code: str) -> Tuple[bool, str]:
        """从统一接口返回的基本信息字符串中解析股票名称"""
        stock_name = "未知"
        if not stock_info or "❌" in stock_info or "未能获取" in stock_info:
    ...
    def _is_historical_data_valid(self, historical_data: str) -> bool:
        historical_data = self._unwrap_stock_data_result(historical_data)
        if not historical_data or "❌" in historical_data or "获取失败" in historical_data:
            return False
        ...
        return len(historical_data) > 50 and any(
            indicator in historical_data for indicator in data_indicators
        )

    def _extract_stock_name_from_historical(self, historical_data: str, stock_code: str) -> str:
        """从历史数据文本中提取股票名称"""
        match = re.search(rf'股票{re.escape(stock_code)}\(([^)]+)\)', historical_data)
        if match:
    ...
    def _try_baostock_china_data(
        ...
        lookback_days: int,
        cache_status: str = "",
    ) -> Optional[StockDataPreparationResult]:
        """Docker 等环境下 AKShare 不可用时，用 BaoStock 直接拉取"""
        try:
            ...
            if ChinaDataSource.BAOSTOCK not in mgr.available_sources:
                return None

            raw = mgr._get_baostock_data(stock_code, start_date, end_date)
            historical_data = self._unwrap_stock_data_result(raw)
            if not self._is_historical_data_valid(historical_data):
                return None

            stock_name = self._extract_stock_name_from_historical(historical_data, stock_code)
            has_basic_info = stock_name != f"股票{stock_code}"
            ...
            if not has_basic_info:
                try:
                    ...
                    info = asyncio.run(provider.get_stock_basic_info(stock_code))
                    if info and info.get("name"):
                        stock_name = info["name"]
                        has_basic_info = True
                except Exception as info_err:
                    logger.debug(f"BaoStock 基本信息获取失败: {info_err}")

            status = (cache_status.rstrip("; ") + "; " if cache_status else "") + f"BaoStock历史数据({lookback_days}天)"
            logger.info(f"✅ [BaoStock兜底] 成功获取 {stock_code} 历史数据")
            return StockDataPreparationResult(
                is_valid=True,
                stock_code=stock_code,
                market_type="A股",
            ...
            logger.warning(f"⚠️ [BaoStock兜底] 获取 {stock_code} 失败: {e}")
            return None

    def _fetch_china_data_once(
        self,
        stock_code: str,
        start_date: str,
        end_date: str,
        ...
        has_basic_info, stock_name = self._parse_stock_name_from_info(stock_info, stock_code)

        historical_data = get_china_stock_data_unified(stock_code, start_date, end_date)
        historical_data = self._unwrap_stock_data_result(historical_data)

        if self._is_historical_data_valid(historical_data):
            if not has_basic_info:
                stock_name = self._extract_stock_name_from_historical(historical_data, stock_code)
                has_basic_info = stock_name != f"股票{stock_code}"
            status = cache_status.rstrip('; ')
            if status:
                status += f"; 历史数据已缓存({lookback_days}天)"
            else:
                status = f"历史数据已获取({lookback_days}天)"
            return StockDataPreparationResult(
                is_valid=True,
                stock_code=stock_code,
                market_type="A股",
        ...
        if not has_basic_info:
            if stock_info and ("❌" in stock_info or "未能获取" in stock_info):
                baostock_result = self._try_baostock_china_data(
                    stock_code, start_date, end_date, lookback_days, cache_status
                )
                if baostock_result:
    ...
    async def _prepare_china_stock_data_async(self, stock_code: str, period_days: int,
                                             analysis_date: str) -> StockDataPreparationResult:
        """预获取A股数据（异步版本），优先直接拉取 API，失败后再同步数据库"""
        import asyncio
        ...
        try:
            logger.info(f"🚀 [A股数据-异步] 优先尝试 API 直接获取: {stock_code}")
            fast_result = await asyncio.to_thread(
                self._fetch_china_data_once,
                stock_code,
            ...
            if fast_result.is_valid:
                logger.info(f"✅ [A股数据-异步] API 直接获取成功: {stock_code} - {fast_result.stock_name}")
                return fast_result

            logger.warning(
                f"⚠️ [A股数据-异步] API 直接获取失败: {fast_result.error_message}，尝试数据库同步后重试"
            )

            cache_status = "API直接获取失败; "
            db_check_result = self._check_database_data(stock_code, extended_start_date_str, end_date_str)
            ...
            if retry_result.is_valid:
                logger.info(f"🎉 [A股数据-异步] 数据准备完成: {stock_code} - {retry_result.stock_name}")
            else:
                logger.error(f"❌ [A股数据-异步] 数据准备失败: {retry_result.error_message}")
            return retry_result
