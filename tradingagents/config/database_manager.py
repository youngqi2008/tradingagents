...
class DatabaseManager:
    ...
    def _load_env_config(self):
        ...
        try:
            ...
            # override=False：Docker environment 注入的变量优先于镜像内 /app/.env
        ...
        # Docker Compose：宿主机 .env 常含 MONGODB_ENABLED=false（本地无 MongoDB），
        # 但 compose 已注入 mongodb/redis 服务名，此时应自动启用数据库
        if parse_bool_env("DOCKER_CONTAINER", False):
            mongo_host = (os.getenv("MONGODB_HOST") or "").strip()
            redis_host = (os.getenv("REDIS_HOST") or "").strip()
            local_hosts = {"localhost", "127.0.0.1", ""}
            if mongo_host and mongo_host not in local_hosts:
                if not self.mongodb_enabled:
                    self.logger.info(
                        f"🔧 [Docker] 检测到 MONGODB_HOST={mongo_host}，自动启用 MongoDB"
                    )
                self.mongodb_enabled = True
            if redis_host and redis_host not in local_hosts:
                if not self.redis_enabled:
                    self.logger.info(
                        f"🔧 [Docker] 检测到 REDIS_HOST={redis_host}，自动启用 Redis"
                    )
