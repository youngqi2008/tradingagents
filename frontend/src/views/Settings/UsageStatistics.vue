          ...
          <div class="cost-statistic">
            ...
            <div class="cost-values">
              <div v-for="(cost, currency) in statistics.cost_by_currency" :key="currency" class="cost-item">
                <span class="cost-amount">{{ cost != null && Number.isFinite(Number(cost)) ? Number(cost).toFixed(4) : '-' }}</span>
    ...
    <el-card style="margin-top: 20px;">
      ...
      <el-table :data="records" style="width: 100%" v-loading="loading">
        ...
        <el-table-column prop="cost" label="成本" width="140" align="right">
          <template #default="{ row }">
            {{ row.cost != null && Number.isFinite(Number(row.cost)) ? Number(row.cost).toFixed(4) : '-' }} {{ getCurrencySymbol(row.currency || 'CNY') }}
          </template>
        </el-table-column>
