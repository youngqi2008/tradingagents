      ...
      setup() {
        ...
        return () => h('div', { style: 'line-height: 2;' }, [
          ...
          recommendation.targetPrice ? h('p', [
            h('strong', '目标价格：'),
            h('span', { style: 'color: #E6A23C;' }, `${recommendation.targetPrice != null && Number.isFinite(Number(recommendation.targetPrice)) ? Number(recommendation.targetPrice).toFixed(2) : '-'}元`),
            h('span', { style: 'color: #909399; font-size: 12px; margin-left: 8px;' }, '(仅供参考)')
          ]) : null,
          h('p', [
            h('strong', '当前价格：'),
            h('span', `${currentPrice != null && Number.isFinite(Number(currentPrice)) ? Number(currentPrice).toFixed(2) : '-'}元`)
          ]),
          ...
          h('p', [
            h('strong', '模型置信度：'),
            h('span', `${recommendation.confidence != null && Number.isFinite(Number(recommendation.confidence)) ? (Number(recommendation.confidence) * 100).toFixed(1) : '-'}%`),
          ...
          recommendation.action === 'buy' ? h('p', { style: 'color: #909399; font-size: 12px; margin-top: 12px;' },
            `可用资金：${availableCash != null && Number.isFinite(Number(availableCash)) ? Number(availableCash).toFixed(2) : availableCash || '0'}元，最大可买：${maxQuantity}股`
          ) : null,
...
.report-detail {
  ...
  .report-content {
    ...
    /* 报告正文 Markdown 与卡片内文字：统一可读色，适配深色/浅色背景 */
    .summary-card .card-header,
    .metrics-card .card-header,
    .modules-card .card-header {
      color: var(--el-text-color-primary);
    }

    .modules-card :deep(.el-tabs__header) {
      background: var(--el-fill-color-blank);
      border-color: var(--el-border-color-lighter);
    }
    .modules-card :deep(.el-tabs__item),
    .modules-card :deep(.el-tabs__nav-wrap::after) {
      color: var(--el-text-color-primary);
      border-color: var(--el-border-color-lighter);
    }
    .modules-card :deep(.el-tabs__item.is-active) {
      color: var(--el-color-primary);
    }
    .modules-card :deep(.el-tabs__content) {
      background: var(--el-fill-color-blank);
      color: var(--el-text-color-primary);
    }
    .modules-card :deep(.el-tabs--border-card) {
      background: var(--el-fill-color-blank);
      border-color: var(--el-border-color-lighter);
    }
