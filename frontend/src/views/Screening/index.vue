    ...
    <el-card v-if="screeningResults.length > 0" class="results-panel" shadow="never">
      ...
      >
        ...
        <el-table-column prop="close" label="当前价格" width="100" align="right">
          <template #default="{ row }">
            <span v-if="row.close != null && row.close !== undefined && Number.isFinite(Number(row.close))">¥{{ Number(row.close).toFixed(2) }}</span>
        ...
        <el-table-column prop="pct_chg" label="涨跌幅" width="100" align="right">
          <template #default="{ row }">
            <span v-if="row.pct_chg !== null && row.pct_chg !== undefined" :class="getChangeClass(row.pct_chg)">
              {{ row.pct_chg != null && row.pct_chg !== undefined && Number.isFinite(Number(row.pct_chg)) ? (row.pct_chg > 0 ? '+' : '') + Number(row.pct_chg).toFixed(2) + '%' : '-' }}
            </span>
            <span v-else class="text-gray-400">-</span>
          </template>
        </el-table-column>
        ...
        <el-table-column prop="pe" label="市盈率" width="130" align="right">
          <template #default="{ row }">
            <span v-if="row.pe">
              {{ row.pe != null && row.pe !== undefined && Number.isFinite(Number(row.pe)) ? Number(row.pe).toFixed(2) : '-' }}
        ...
        <el-table-column prop="pb" label="市净率" width="130" align="right">
          <template #default="{ row }">
            <span v-if="row.pb">
              {{ row.pb != null && row.pb !== undefined && Number.isFinite(Number(row.pb)) ? Number(row.pb).toFixed(2) : '-' }}
              <el-tag v-if="row.pe_is_realtime" type="success" size="small" style="margin-left: 4px">实时</el-tag>
            </span>
            <span v-else class="text-gray-400">-</span>
          </template>
        ...
        <el-table-column prop="roe" label="ROE(%)" width="110" align="right">
          <template #default="{ row }">
            <span v-if="row.roe !== null && row.roe !== undefined && Number.isFinite(Number(row.roe))">{{ Number(row.roe).toFixed(2) }}%</span>
            <span v-else class="text-gray-400">-</span>
          </template>
        </el-table-column>
...
const formatMarketCap = (marketCap: number | null | undefined) => {
  if (marketCap == null || !Number.isFinite(Number(marketCap))) {
    return '-'
  }
