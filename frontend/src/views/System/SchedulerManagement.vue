            ...
            <el-table-column prop="timestamp" label="执行时长" width="180">
              <template #default="{ row }">
                <span v-if="row.execution_time !== undefined && row.execution_time !== null">
                  {{ row.execution_time != null && Number.isFinite(Number(row.execution_time)) ? Number(row.execution_time).toFixed(2) : '-' }}秒
                </span>
    ...
    >
      <el-descriptions v-if="currentExecution" :column="1" border>
        ...
        <el-descriptions-item label="错误堆栈" v-if="currentExecution.traceback">
          <pre style="max-height: 300px; overflow-y: auto; background: #2a2a2a; color: #ffffff; padding: 12px; border-radius: 4px; border: 1px solid #333333;">{{ currentExecution.traceback }}</pre>
        </el-descriptions-item>
      </el-descriptions>
