...
from app.stock_agent.dsa_storage import persist_llm_usage as _persist_usage
...
class AgentExecutor:
    ...
    def chat(self, message: str, session_id: str, progress_callback: Optional[Callable] = None, context: Optional[Dict[str, Any]] = None) -> AgentResult:
        ...
        """
        from app.stock_agent.agent.conversation import conversation_manager
    ...
    def chat_with_history(
        ...
        """问股：使用外部会话历史（MySQL mp_chat_messages），不写入 conversation_messages。"""
        start_time = time.time()
        tool_calls_log: List[Dict[str, Any]] = []
        total_tokens = 0
