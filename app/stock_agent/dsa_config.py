...
_MANAGED_LITELLM_KEY_PROVIDERS = {"gemini", "vertex_ai", "anthropic", "openai", "deepseek", "dashscope"}


def _filter_valid_api_keys(keys: List[str]) -> List[str]:
    """过滤占位符与无效 API Key。"""
    from app.utils.api_key_utils import is_valid_api_key

    return [k for k in keys if is_valid_api_key(k)]
...
class Config:
    ...
    dashscope_api_keys: List[str] = field(default_factory=list)
    ...
    # === 数据库配置 ===
    database_path: str = "./data/stock_analysis.db"  # 已废弃，保留兼容；实际使用 MYSQL_*
    ...
    def _load_from_env(cls) -> 'Config':
        ...
        # DASHSCOPE_API_KEYS > DASHSCOPE_API_KEY / QWEN_API_KEY
        _dashscope_keys_raw = os.getenv('DASHSCOPE_API_KEYS', '')
        dashscope_api_keys = [k.strip() for k in _dashscope_keys_raw.split(',') if k.strip()]
        if not dashscope_api_keys:
            for _env_name in ('DASHSCOPE_API_KEY', 'QWEN_API_KEY'):
                _single_dashscope = os.getenv(_env_name, '').strip()
                if _single_dashscope:
                    dashscope_api_keys = [_single_dashscope]
                    break

        gemini_api_keys = _filter_valid_api_keys(gemini_api_keys)
        anthropic_api_keys = _filter_valid_api_keys(anthropic_api_keys)
        openai_api_keys = _filter_valid_api_keys(openai_api_keys)
        deepseek_api_keys = _filter_valid_api_keys(deepseek_api_keys)
        dashscope_api_keys = _filter_valid_api_keys(dashscope_api_keys)
        ...
        if not litellm_model:
            _dashscope_model_name = (
                os.getenv('DASHSCOPE_MODEL') or os.getenv('QWEN_MODEL') or 'qwen-plus'
            ).strip()
            ...
            if dashscope_api_keys:
                litellm_model = (
                    f'dashscope/{_dashscope_model_name}'
                    if '/' not in _dashscope_model_name
                    else _dashscope_model_name
                )
    ...
    def _legacy_keys_to_model_list(
        ...
        dashscope_keys: Optional[List[str]] = None,
    ) -> List[Dict[str, Any]]:
        ...
        # Gemini keys
        for k in _filter_valid_api_keys(gemini_keys):
            model_list.append({
        ...
        # Anthropic keys
        for k in _filter_valid_api_keys(anthropic_keys):
            model_list.append({
        ...
        for k in _filter_valid_api_keys(openai_keys):
        ...
        for k in _filter_valid_api_keys(deepseek_keys or []):
            model_list.append({
        ...
        # DashScope / 通义千问
        for k in _filter_valid_api_keys(dashscope_keys or []):
            model_list.append({
                'model_name': '__legacy_dashscope__',
                'litellm_params': {
                    'model': '__legacy_dashscope__',
                    'api_key': k,
                },
            })
    ...
    def get_db_url(self) -> str:
        """获取 SQLAlchemy 数据库连接 URL（MySQL，与主项目共用）"""
        from app.core.mysql_db import get_sync_mysql_url

        return get_sync_mysql_url()
...
def get_api_keys_for_model(model: str, config: Config) -> List[str]:
    ...
    if provider in {"gemini", "vertex_ai"}:
        return _filter_valid_api_keys(config.gemini_api_keys)
    if provider == "anthropic":
        return _filter_valid_api_keys(config.anthropic_api_keys)
    if provider == "deepseek":
        return _filter_valid_api_keys(config.deepseek_api_keys)
    if provider == "dashscope":
        return _filter_valid_api_keys(config.dashscope_api_keys)
    if provider == "openai":
        return _filter_valid_api_keys(config.openai_api_keys)
