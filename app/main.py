"""
Ares 智能研报 v1.0.0-preview FastAPI Backend
主应用程序入口
...
from app.routers import auth_db as auth, analysis, screening, queue, sse, health, favorites, config, reports, database, operation_logs, tags, tushare_init, akshare_init, baostock_init, historical_data, multi_period_sync, financial_data, news_data, social_media, internal_messages, usage_statistics, model_capabilities, cache, logs, admin_ops as admin_ops_router, ares_signal as ares_signal_router
from app.routers.mp import router as mp_router
from app.routers.mp import payment_notify as mp_payment_notify_router
...
async def _print_config_summary(logger):
    ...
    try:
        logger.info("=" * 70)
        logger.info("📋 Ares 智能研报 Configuration Summary")
        logger.info("=" * 70)
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        await role_service.init_default_roles()
        from app.services.benefit_service import init_default_level_campaigns
        await init_default_level_campaigns()
    except Exception as e:
        logger.warning(f"⚠️ MySQL/会员模块初始化失败: {e}")
    ...
    try:
        ...
        # 自选股日报 → 小程序站内信（替代 DSA 企业微信 STOCK_LIST）
        async def run_favorites_digest():
            try:
                logger.info("📬 开始自选股日报推送（小程序站内信）...")
                ...
                logger.info(
                    f"✅ 自选日报完成: 用户{result.get('users_total')} "
                    f"推送{result.get('sent')} 跳过{result.get('skipped')}"
                )
            except Exception as e:
                logger.error(f"❌ 自选日报推送失败: {e}", exc_info=True)

        scheduler.add_job(
            run_favorites_digest,
            CronTrigger.from_crontab(settings.FAVORITES_DIGEST_CRON, timezone=settings.TIMEZONE),
            id="favorites_digest",
            name="自选股日报推送（小程序站内信）",
        )
        if not settings.FAVORITES_DIGEST_ENABLED:
            scheduler.pause_job("favorites_digest")
            logger.info(f"⏸️ 自选日报推送已添加但暂停: {settings.FAVORITES_DIGEST_CRON}")
        else:
            logger.info(f"📬 自选日报推送已配置: {settings.FAVORITES_DIGEST_CRON}")

        # 大盘复盘 → 全体小程序（移植 DSA 企微下午推送）
        async def run_market_review():
            try:
                logger.info("📊 开始大盘复盘推送（全体小程序）...")
                from app.services.market_review_service import market_review_service

                result = await market_review_service.run_and_push()
                logger.info(
                    f"✅ 大盘复盘完成: 指数{result.get('indices_count')} "
                    f"涨{result.get('up_count')}/跌{result.get('down_count')} "
                    f"通知={result.get('notification_id')}"
                )
            except Exception as e:
                logger.error(f"❌ 大盘复盘推送失败: {e}", exc_info=True)

        scheduler.add_job(
            run_market_review,
            CronTrigger.from_crontab(settings.MARKET_REVIEW_CRON, timezone=settings.TIMEZONE),
            id="market_review",
            name="大盘复盘推送（全体小程序）",
        )
        if not settings.MARKET_REVIEW_ENABLED:
            scheduler.pause_job("market_review")
            logger.info(f"⏸️ 大盘复盘推送已添加但暂停: {settings.MARKET_REVIEW_CRON}")
        else:
...
app.include_router(mp_payment_notify_router.router)
app.include_router(admin_ops_router.router)
app.include_router(ares_signal_router.router)
