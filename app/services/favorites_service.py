...
class FavoritesService:
    ...
    @classmethod
    def mp_user_key(cls, user_id: str | int) -> str:
        """小程序用户在 user_favorites 中的命名空间，避免与 Web 用户 ID 冲突。"""
        return f"{cls.MP_USER_PREFIX}{user_id}"

    @classmethod
    def parse_mp_user_id(cls, key: str) -> Optional[int]:
        if not key or not str(key).startswith(cls.MP_USER_PREFIX):
            return None
        try:
            return int(str(key)[len(cls.MP_USER_PREFIX) :])
        except (TypeError, ValueError):
            return None

    async def get_all_unique_stock_codes(self) -> List[str]:
        """聚合全部用户（Web + 小程序）自选股代码，去重。供新闻同步等任务使用。"""
        db = await self._get_db()
        codes: set = set()
        ...
        async for user in users_cursor:
            for fav in user.get("favorite_stocks", []) or []:
                code = fav.get("stock_code")
                if code:
                    codes.add(str(code).zfill(6) if str(code).isdigit() else str(code))

        fav_cursor = db.user_favorites.find(
            {"favorites": {"$exists": True, "$ne": []}},
            {"favorites.stock_code": 1, "_id": 0},
        )
        async for doc in fav_cursor:
    ...
    async def list_mp_favorites_by_user(self) -> Dict[int, List[Dict[str, Any]]]:
        """返回 {mysql_user_id: [favorite_items]}，仅小程序命名空间。"""
        db = await self._get_db()
        out: Dict[int, List[Dict[str, Any]]] = {}
        cursor = db.user_favorites.find(
            {"user_id": {"$regex": f"^{self.MP_USER_PREFIX}"}},
            {"user_id": 1, "favorites": 1},
        )
        async for doc in cursor:
            uid = self.parse_mp_user_id(doc.get("user_id") or "")
            if not uid:
                continue
            items = [self._format_favorite(f) for f in (doc.get("favorites") or [])]
            if items:
                out[uid] = items
        return out

    async def list_favorites_admin(
        self,
        *,
        ...
        """运营侧：扁平列出自选订阅。"""
        db = await self._get_db()
        query: Dict[str, Any] = {"favorites": {"$exists": True, "$ne": []}}
        if user_id:
            # 同时兼容原始 id / mp:id
            keys = [user_id]
            if not str(user_id).startswith(self.MP_USER_PREFIX):
                keys.append(self.mp_user_key(user_id))
            query["user_id"] = {"$in": keys}
        elif mp_only:
            query["user_id"] = {"$regex": f"^{self.MP_USER_PREFIX}"}

        cursor = db.user_favorites.find(query).sort("updated_at", -1)
        docs = await cursor.to_list(length=None)
        rows: List[Dict[str, Any]] = []
        for doc in docs:
            uid = doc.get("user_id") or ""
            mp_uid = self.parse_mp_user_id(uid)
            for fav in doc.get("favorites") or []:
                code = fav.get("stock_code") or ""
                if stock_code and str(code) != str(stock_code) and str(code).zfill(6) != str(stock_code).zfill(6):
                    continue
                ...
                rows.append(
                    {
                        "user_id": str(mp_uid) if mp_uid else uid,
                        "storage_user_id": uid,
                        "is_mp_user": mp_uid is not None,
                        "stock_code": code,
                        "stock_name": fav.get("stock_name") or "",
                        "market": fav.get("market") or "A股",
                        "notes": fav.get("notes") or "",
