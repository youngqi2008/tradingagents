...
def _resolve_llm_api_key(model_api_key, provider_api_key, provider: str):
    """解析 LLM API Key：模型配置 > 厂家配置 > 环境变量"""
    from app.utils.api_key_utils import resolve_llm_api_key
    return resolve_llm_api_key(model_api_key, provider_api_key, provider)
    ...
    try:
        ...
        if doc and "llm_configs" in doc:
            ...
            for config_dict in llm_configs:
                if config_dict.get("model_name") == model_name:
                    ...
                    # 🔥 确定 API Key（优先级：模型配置 > 厂家配置 > 环境变量；跳过占位符）
                    provider_api_key = provider_doc.get("api_key") if provider_doc else None
                    api_key = _resolve_llm_api_key(model_api_key, provider_api_key, provider)
                    if api_key:
                        logger.info(f"✅ [同步查询] 已解析 {provider} 的有效 API Key")
                    else:
        ...
        try:
            ...
            api_key = _resolve_llm_api_key(
                None, provider_doc.get("api_key") if provider_doc else None, provider
            )
            if api_key:
                logger.info(f"✅ [同步查询] 已解析厂家 {provider} 的有效 API Key")
...
def _get_default_backend_url(provider: str) -> str:
    ...
    url = default_urls.get(normalized, "https://dashscope.aliyuncs.com/compatible-mode/v1")
...
def create_analysis_config(
    ...
    config["llm_provider"] = _normalize_llm_provider(llm_provider)
...
class SimpleAnalysisService:
    ...
    async def execute_analysis_background(
        ...
        try:
            ...
            if not validation_result.is_valid:
                ...
                if "历史数据" in error_message:
                    error_title = "❌ 无法获取行情数据"
                elif "基本信息" in error_message or "不存在" in error_message:
                    error_title = "❌ 股票代码无效"
                else:
                    error_title = "❌ 数据准备失败"
    ...
    def _run_analysis_sync(
        ...
        try:
            ...
            config["quick_api_key"] = quick_provider_info.get("api_key") or config.get("quick_api_key")
            config["deep_api_key"] = deep_provider_info.get("api_key") or config.get("deep_api_key")
