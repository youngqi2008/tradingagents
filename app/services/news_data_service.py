...
class NewsDataService:
    ...
    def _standardize_news_data(
        ...
        standardized = {
            ...
            # 新闻内容（去掉东财标题中的 <em> 等 HTML）
            "title": self._strip_html(news_data.get("title", "")),
            "content": self._strip_html(news_data.get("content", "")),
            "summary": self._strip_html(news_data.get("summary", "")),
    ...
    def _strip_html(self, text: Any) -> str:
        """清理新闻标题/正文中的 HTML 标签（如东财高亮 <em>）。"""
        if text is None:
            return ""
        s = unescape(str(text))
        s = self._HTML_TAG_RE.sub("", s)
