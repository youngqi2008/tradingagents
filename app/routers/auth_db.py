...
from app.models.sql.converters import user_to_response
...
async def login(payload: LoginRequest, request: Request):
    ...
    try:
        ...
        # 生成 token（access / refresh 类型区分）
        token = AuthService.create_access_token(sub=user.username, token_type="access")
...
async def refresh_token(payload: RefreshTokenRequest):
    ...
    try:
        ...
        # 验证 refresh token（禁止用 access token 换发）
        token_data = AuthService.verify_token(payload.refresh_token, expected_type="refresh")
...
async def update_me(
    ...
    try:
        ...
        # 仅返回安全字段，禁止泄露 hashed_password 等
        return {
            "success": True,
            "data": user_to_response(updated_user).model_dump(),
            "message": "用户信息更新成功"
        }
    except HTTPException:
        raise
