    ...
    try:
        ...
        try:
            ...
            for provider in providers:
                ...
                normalized_name = provider.name.lower()
                env_keys = [f"{provider.name.upper()}_API_KEY"]
                if normalized_name in ("qwen", "dashscope", "alibaba", "tongyi"):
                    env_keys = ["DASHSCOPE_API_KEY", "QWEN_API_KEY"] + env_keys

                existing_env_value = None
                for env_key in env_keys:
                    val = os.getenv(env_key)
                    if is_valid_api_key(val):
                        existing_env_value = val
                        break

                # 检查环境变量是否已存在且有效（不是占位符）
                if is_valid_api_key(existing_env_value):
                    for env_key in env_keys:
                        if env_key in ("DASHSCOPE_API_KEY", "QWEN_API_KEY") or env_key == f"{provider.name.upper()}_API_KEY":
                            os.environ[env_key] = existing_env_value.strip()
                    logger.info(f"  ✓ 使用 .env 文件中的 API Key ({provider.name}, 长度: {len(existing_env_value)})")
                    bridged_count += 1
                elif is_valid_api_key(provider.api_key):
                    resolved_key = provider.api_key.strip()
                    for env_key in env_keys:
                        os.environ[env_key] = resolved_key
                    logger.info(f"  ✓ 使用数据库厂家配置的 API Key ({provider.name}, 长度: {len(resolved_key)})")
                    bridged_count += 1
                else:
                    logger.debug(f"  ⏭️  {provider.name} 未配置有效的 API Key")
