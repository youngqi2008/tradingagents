...
class StartupValidator:
    ...
    REQUIRED_CONFIGS = [
        ...
        ConfigItem(
            key="JWT_SECRET",
            level=ConfigLevel.REQUIRED,
            description="JWT密钥（用于生成认证令牌，至少32字符且非默认值）",
            example="use-a-long-random-secret-at-least-32-chars",
            validator=lambda v: (
                len(v) >= 32
                and v
                not in {
    ...
    def _check_security_configs(self):
        ...
        if jwt_secret in weak_secrets or len(jwt_secret) < 32:
            cfg = ConfigItem(
                key="JWT_SECRET",
                level=ConfigLevel.REQUIRED,
                description="JWT密钥必须足够强（>=32 且非默认值）",
                example="use-a-long-random-secret-at-least-32-chars",
            )
            self.result.invalid_configs.append((cfg, "JWT_SECRET 过弱或为默认值"))
            logger.error("❌ JWT_SECRET 过弱或为默认值，拒绝启动")

        # 生产环境强制关闭危险开关
        debug = os.getenv("DEBUG", str(settings.DEBUG)).lower() in ("true", "1", "yes", "on")
        if not debug:
            logger.info("ℹ️  生产环境模式（DEBUG=false）")
            if settings.WECHAT_ALLOW_DEV_LOGIN:
                self.result.warnings.append(
                    "生产环境应关闭 WECHAT_ALLOW_DEV_LOGIN（已在配置层强制关闭）"
                )
            if not (settings.ARES_SIGNAL_TOKEN or "").strip():
                self.result.warnings.append(
                    "未配置 ARES_SIGNAL_TOKEN：/api/ares/signal 将返回 503"
                )
        else:
            logger.info("ℹ️  开发环境模式（DEBUG=true）")
            if settings.WECHAT_ALLOW_DEV_LOGIN:
                self.result.warnings.append(
                    "已开启 WECHAT_ALLOW_DEV_LOGIN，切勿用于生产"
                )
    ...
    def _print_validation_result(self):
        """输出验证结果"""
        logger.info("\n" + "=" * 70)
        logger.info("Ares 智能研报 Configuration Validation Result")
        logger.info("=" * 70)
