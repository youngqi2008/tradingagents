...
Ares 智能研报采用本地构建Docker镜像的方式，而不是提供预构建镜像。本文档详细说明了Docker镜像的构建过程、优化方法和常见问题解决方案。
...
# 分析构建层
docker history Ares 智能研报:latest

# 查看镜像大小
docker images Ares 智能研报
```
...
```bash
# 为镜像打标签
docker build -t Ares 智能研报:v0.1.7 .
docker build -t Ares 智能研报:latest .

# 推送到私有仓库 (可选)
docker tag Ares 智能研报:latest your-registry/Ares 智能研报:latest
docker push your-registry/Ares 智能研报:latest
```
