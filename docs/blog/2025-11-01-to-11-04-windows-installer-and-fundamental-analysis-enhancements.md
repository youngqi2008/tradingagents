...
; Ares 智能研报 Windows 安装器脚本

!include "MUI2.nsh"

; 基本信息
Name "Ares 智能研报"
OutFile "Ares 智能研报-Setup.exe"
InstallDir "$PROGRAMFILES64\Ares 智能研报"
...
Section "主程序" SecMain
    ...
    ; 创建快捷方式
    CreateDirectory "$SMPROGRAMS\Ares 智能研报"
    CreateShortcut "$SMPROGRAMS\Ares 智能研报\Ares 智能研报.lnk" "$INSTDIR\start.bat"
    CreateShortcut "$DESKTOP\Ares 智能研报.lnk" "$INSTDIR\start.bat"
    
    ; 写入卸载信息
    WriteUninstaller "$INSTDIR\Uninstall.exe"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Ares 智能研报" \
                     "DisplayName" "Ares 智能研报"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Ares 智能研报" \
...
Section "Uninstall"
    ...
    ; 删除快捷方式
    Delete "$DESKTOP\Ares 智能研报.lnk"
    RMDir /r "$SMPROGRAMS\Ares 智能研报"
    
    ; 删除注册表
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\Ares 智能研报"
SectionEnd
```
...
def build_installer():
    ...
    if result.returncode == 0:
        print("✅ 安装器构建成功！")
        print(f"📦 输出文件: Ares 智能研报-Setup.exe")
    else:
        print("❌ 安装器构建失败")
        print(result.stderr)
        sys.exit(1)
